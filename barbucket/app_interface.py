...
from barbucket.tv_details import TvDetailsDatabase, TvDetailsFile
...
class AppInterface():
    ...
    def create_contract(self, contract_type_from_listing, broker_symbol, exchange, currency):
        ...
        tws = Tws()
        tws.connect()
        details = tws.download_contract_details(
            contract_type_from_listing=contract_type_from_listing,
            broker_symbol=broker_symbol,
            exchange=exchange,
            currency=currency)
        tws.disconnect()
        # Todo: Exception to make sure, disconnect is performed

        if details == None:
            print("Error. No details returned from TWS. Aborting contract.")
            return
        
        # Store contract into db
        contract = {
            'contract_type_from_listing': contract_type_from_listing,
            'contract_type_from_details': details.stockType,
            'exchange_symbol': details.contract.localSymbol, 
            'broker_symbol': broker_symbol,
        ...
        contract_id = contracts_database.create_contract_entry(
            contract=contract)

        # Create empty entry for quotes status
        quotes_status_db = QuotesStatusDatabase()
        quotes_status_db.create_empty_quotes_status(contract_id=contract_id)

        # Create empty entry for tv details
        tv_details_db = TvDetailsDatabase()
        tv_details_db.create_empty_entry(contract_id=contract_id)
