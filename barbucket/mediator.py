...
class Mediator():
    ...
    def __init__(
        ...
        self.__ib_details_db = ib_details_db
        self.__ib_details_db.mediator = self
    ...
    def notify(self, sender: BaseComponent, action: str, parameters: dict) -> object:
        if action == "get_contracts":
            result = self.__contracts_db.get_contracts(
                filters=parameters['filters'],
                return_columns=parameters['return_columns'])
            return result
        if action == "get_db_connection":
            result = self.__db_connector.connect()
            return result
        if action == "close_db_connection":
            self.__db_connector.disconnect(conn=parameters['conn'])
        if action == "decode_exchange_tv":
            result = self.__db_connector.connect(
